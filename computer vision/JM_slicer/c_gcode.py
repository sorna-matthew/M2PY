def generate_gcode(route_xy, layer_thickness, gcode_rel, subdir_output, file):
    ...
    import os
    f=open(os.path.splitext(os.path.dirname(os.path.realpath(__file__)) + "\\" + subdir_output +"\\" + os.path.basename(file))[0] + '.cnc','w') 
    for i in gcode: 
        f.write(i + '\n') 
    f.close()
